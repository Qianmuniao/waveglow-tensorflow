...
class multiproc_reader():
    ...
    def load_metadata(self):
        ...
        self.metadata = self.metadata[: int(
            len(self.metadata) * args.valsplit)]
        self.n_examples = len(self.metadata)
        print("Total number of audio/text pair for training: %r" %
              self.n_examples)
    ...
    def main_proc(self):  # , cnt):
        ...
        while True:
            ...
            for _ in range(args.batch_size):
                ...
                melname, wavname = cmetadata[1], cmetadata[0]
                mel, audio = padtomaxlen(np.load(melname), np.load(wavname))
                mels.append(mel)
                wavs.append(audio)
            mels = np.transpose(np.array(mels), axes=[0, 2, 1])
            wavs = np.reshape(np.array(wavs), [
                              args.batch_size, args.wav_time_step // args.squeeze_size, args.squeeze_size])
            wavs = np.transpose(wavs, axes=[0, 2, 1])
            self.queue.put([mels, wavs])

    def start_enqueue(self, num_proc=1):  # multiprocessing.cpu_count() // 2):
        if args.num_proc is not None:
            num_proc = args.num_proc
        procs = []
        for _ in range(num_proc):
            # , args=(self.cnt,))
            p = multiprocessing.Process(target=self.main_proc)
            p.start()
            procs.append(p)
        return procs
    ...
    def load_metadata(self):
        with open(args.metadata_dir, 'r', encoding='utf-8') as f:
            ...
            self.metadata = self.metadata[int(
                len(self.metadata) * args.valsplit):]
        self.n_examples = len(self.metadata)
        print("Total number of audio/text pair for validation: %r" %
              self.n_examples)
