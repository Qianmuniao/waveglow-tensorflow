...
class WaveGlow():
    ...
    def build_model(self):
        ...
        ########INFERENCE#########
        self.output = tf.truncated_normal([tf.shape(self.conditions)[0], args.output_remain, tf.shape(self.conditions)[2]], dtype=tf.float32)
        for i in reversed(range(args.n_flows)):
            if (i + 1) % args.early_output_every == 0 and (i + 1) != args.n_flows:
                self.newz = tf.truncated_normal([tf.shape(self.conditions)[0], args.early_output_size, tf.shape(self.conditions)[2]], stddev=args.infer_sigma)
    ...
    def train(self):
        ...
        self.lr = tf.train.exponential_decay(args.lr, self.global_step, args.lr_decay_steps, args.lr_decay_rate)
        ...
        try:
            for epoch in range(start_epoch, args.epoch):
                ...
                loss_names = ["Total Loss", "LogS Loss", "LogDet Loss", "Prior Loss"]
                ...
                for batch in tqdm(range(num_batch)):
                    ...
                    _, loss, logs_loss, logdet_loss, prior_loss, summary, step = self.sess.run(
                        [self.op, self.loss, self.logs_loss, self.logdet_loss, self.prior_loss, self.merged, self.global_step], feed_dict=feed_dict)
