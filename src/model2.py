...
class WaveGlow():
    ...
    def build_model(self):
        self.global_step = tf.get_variable(
            'global_step', initializer=0, dtype=tf.int32, trainable=False)
        ...
        self.conditions = mydeconv1d(self.mels, args.n_mel, filter_size=args.step_per_mel*4,
                                     stride=args.step_per_mel, scope='upsample', reuse=False)
        self.conditions = tf.transpose(self.conditions, perm=[0, 2, 1])
        self.conditions = tf.reshape(self.conditions, [-1, tf.shape(self.conditions)[
                                     1] // args.squeeze_size, args.squeeze_size * args.n_mel])
        self.conditions = tf.transpose(self.conditions, perm=[0, 2, 1])
        self.z = []
        self.layer = self.wavs
        self.logdets, self.logss = 0, 0
        for i in range(args.n_flows):
            self.layer, logs, logdet = conv_afclayer(
                self.layer, self.conditions, reverse=False, scope='afc_'+str(i+1), reuse=False)
            self.logdets += logdet
            self.logss += logs
        ...
        self.prior_loss = tf.reduce_sum(
            self.z ** 2 / (2 * args.sigma ** 2)) / total_size
...
########INFERENCE#########
        self.output = tf.truncated_normal([tf.shape(self.conditions)[
                                          0], args.output_remain, tf.shape(self.conditions)[2]], dtype=tf.float32)
        for i in reversed(range(args.n_flows)):
            if (i + 1) % args.early_output_every == 0 and (i + 1) != args.n_flows:
                self.newz = tf.truncated_normal([tf.shape(self.conditions)[
                                                0], args.early_output_size, tf.shape(self.conditions)[2]], stddev=args.infer_sigma)
                self.output = tf.concat([self.newz, self.output], axis=1)
            self.output = conv_afclayer(
                self.output, self.conditions, reverse=True, scope='afc_'+str(i+1), reuse=tf.AUTO_REUSE)
########INFERENCE#########
    ...
    def train(self):
        ...
        with tf.name_scope("summaries"):
            self.s_logdet_loss = tf.summary.scalar(
                'logdet_loss', self.logdet_loss)
            self.s_logs_loss = tf.summary.scalar('logs_loss', self.logs_loss)
            self.s_prior_loss = tf.summary.scalar(
        ...
        try:
            for epoch in range(start_epoch, args.epoch):
                ...
                for batch in tqdm(range(num_batch),ncols=50):
                    ...
                    buffers.put([loss, logs_loss, logdet_loss,
                                 prior_loss], [0, 1, 2, 3])
